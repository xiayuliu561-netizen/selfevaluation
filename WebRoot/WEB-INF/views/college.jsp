...
<body style="padding-top: 0px; background-color: #f1f4f5">
            ...
            <div class="panel-body page-toolbar">
                <form id="formSearch" class="form-inline" method="post" >
                    <div class="form-group">
                            ...
                            <span class="help-block" style="display:inline-block;margin:0;color:#777;">学院信息可在学生名单导入时自动补全，也可在此维护</span>
                    </div>
                </form>
            </div>
        </div> 
				        ...
				        <div class="panel panel-info" >
				            ...
				            <div class="panel-body">
			                    <div class="form-group" style="margin-top:5px; margin-right: 5px;" >
									...
									<div class="col-sm-4" >
										<input id="collegeName" name="collegeName" type="text" class="form-control input-sm" placeholder="" autocomplete="off">
	...
	<script>
		...
		var TableInit = function () {
		    ...
		    oTableInit.Init = function () {
		        $('#table').bootstrapTable({
		            url: '<%=basePath%>college/listdata.html',         //请求后台的URL（*）
		            method: 'get',                      //请求方式（*）
		            ...
		            columns: [{
		                ...
		                formatter:function(value,row,index){
						    var element = 
							    "<button title='修改' class='btn btn-warning btn-xs' onclick='toupdate(\""+row.id +"\",\""+row.collegeName +"\");'><i class='icon fa-edit'></i></button> "+ 
		...
		//修改弹窗
		function toupdate(id, collegeName){
		...
		//修改
			$('#update').on('click', function () {
				if(formcheck()){
					$.ajax({
		                cache: true,
		                type: "POST",
		                url:"<%=basePath %>college/update.html",
	                ...
	                success: function(data) {
	                	...
	                	if("修改成功"== data){
	                		window.location = "<%=basePath%>college/list.html";
	                	}
	                }
	            });
			}
			...
			//删除按钮
			function del(id){
				window.SelfEvalUI.confirmDanger("确定要删除该学院信息？", "删除学院可能影响关联班级、教师或学生数据展示。", function(){
					$.ajax({
		                cache: true,
		                type: "POST",
		                url:"<%=basePath%>college/del.html",
	                data:{"id":id},
	                error: function(request) {
	                    parent.window.toastralert('error','网络连接错误，请联系管理员');
	                },
		...
		$('#btn_batch_delete').on('click', function () {
			...
			if(rows.length == 0){
				parent.window.toastralert('warning','请选择要删除的学院');
				return;
			}
			var ids = [];
			for(var i=0; i<rows.length; i++){
				...
				window.SelfEvalUI.confirmDanger("确定要删除选中的 " + ids.length + " 条学院信息？", "批量删除后不可恢复，请确认已选记录。", function(){
					$.ajax({
		                cache: true,
		                type: "POST",
		                url:"<%=basePath%>college/dels.html",
	                data:{"ids":ids.join(',')},
	                error: function(request) {
		...
		function formcheck(){
			...
			if(collegeName == null || collegeName == ''){
				parent.window.toastralert('warning','学院不能为空！');
				isvalid = false;
			}
			return isvalid;
		}
