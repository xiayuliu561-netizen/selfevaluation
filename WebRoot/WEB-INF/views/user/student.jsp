...
<body style="padding-top: 0px; background-color: #f1f4f5">
    ...
    <div class="panel introduce-info" id="maindiv" style="margin-top: -22px;">
		<div class="panel panel-default" >
            <div class="panel-body student-toolbar">
                <form id="formSearch" class="form-inline" method="post" >
                    <div class="form-group">
                            <input type="text" class="form-control input-sm" id="queryname" name="queryname"  placeholder="用户名/学号/姓名/邮箱/电话">
                            <button type="button" style="margin-left:10px" id="btn_query" class="btn btn-primary btn-sm">查询</button>
							<span class="help-block" style="display:inline-block;margin:0;color:#777;">学生名单请在课程管理中按课程导入或维护</span>
                    </div>
                </form>
            </div>
        </div> 
  
	    <div class="panel-body student-table-wrap">
				        ...
				        <div class="panel panel-info" >
				            ...
				            <div class="panel-body">
				            	<div class="form-group" >
									...
									<div class="col-sm-4" >
										<select class="form-control input-sm" id="collegeId" name="collegeId" onchange="changeCollege(this.value)">
			                    ...
			                    <div class="form-group" >
									...
									<div class="col-sm-4" >
										<input id="no" name="no" type="text" class="form-control input-sm" placeholder="请输入学号" autocomplete="off">
										<input id="id" name="id" type="hidden" >
										<input id="isadmin" name="isadmin" value="2" type="hidden" >
									</div>
			                    ...
			                    <div class="form-group" >
									...
									<div class="col-sm-4" >
										<input id="username" name="username" type="text" class="form-control input-sm" readonly="readonly" placeholder="请输入用户名" autocomplete="off">
									</div>
			                    </div>
			                    <div class="form-group" >
	...
	<script>
		...
		//修改
				...
				if(formcheck()){
	                ...
	                success: function(data) {
	                	if("修改成功"== data){
	                		...
	                		window.location = "<%=basePath%>user/studentlist.html";
	                	}else{
			...
			//删除按钮
			function del(id){
				window.SelfEvalUI.confirmDanger("确定要删除该学生信息？", "删除后该学生账号和相关列表记录将不可恢复。", function(){
					$.ajax({
	                cache: true,
	                type: "POST",
		...
		$('#btn_batch_delete').on('click', function () {
			...
			if(rows.length == 0){
				parent.window.toastralert('warning','请选择要删除的学生');
				return;
			}
			var ids = [];
			for(var i=0; i<rows.length; i++){
			...
			window.SelfEvalUI.confirmDanger("确定要删除选中的 " + ids.length + " 条学生信息？", "批量删除后不可恢复，请确认已选记录。", function(){
				$.ajax({
	                cache: true,
	                type: "POST",
		...
		function formcheck(){
			...
			if($('#deptId').val()== null || $('#deptId').val()== ''){
				parent.window.toastralert('error','班级不能为空');
				isvalid = false;
			}
			if($('#no').val()== null || $('#no').val()== ''){
				parent.window.toastralert('error','学号不能为空');
				isvalid = false;
			}
