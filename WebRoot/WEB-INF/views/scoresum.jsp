            ...
            <div class="panel-body">
                <form id="formSearch" class="form-inline" method="post" >
                    <div class="form-group" style="margin-top:5px; margin-bottom:5px; margin-left: 5px;">
				        <select class="form-control input-sm"  id="queryname" name="queryname" style="width: 140px;" >
                    		<option value="">选择学生</option>
                    		<c:forEach var="user" items="${userlist}" varStatus="status">
                    			<option value="${user.id}">${user.name}-${user.no}</option>
                    		</c:forEach>
                    	</select>
				       	<button type="button" style="margin-left:5px" id="btn_query" class="btn btn-primary btn-sm">查询</button>
			        </div>
                </form>
            </div>
        </div> 
		...
		var TableInit = function () {
		    ...
		    oTableInit.Init = function () {
		        $('#table').bootstrapTable({
		            url: '<%=basePath%>score/sumlistdata.html',         //请求后台的URL（*）
		            method: 'get',                      //请求方式（*）
